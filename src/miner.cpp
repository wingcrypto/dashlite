...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("DashliteMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("DashliteMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Dashlite-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running DashliteMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
